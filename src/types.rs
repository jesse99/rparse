...
struct State {file: @~str, text: @[char], index: uint, line: int}
...
struct Succeeded<T: Copy Owned> {new_state: State, value: T}

/// old_state should be identical to the input state. err_state is where 
/// the error happened.
struct Failed {old_state: State, err_state: State, mesg: @~str}
